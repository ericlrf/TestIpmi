...
public class GetFruInfoRunner {
	...
	/**
	 * This is the value of Last Record ID (FFFFh). In order to retrieve the
	 * full set of SDR records, client must repeat reading SDR records until
	 * MAX_REPO_RECORD_ID is returned as next record ID. For further information
	 * see section 33.12 of the IPMI specification ver. 2.0
	 */
	...
	/**
	 * Size of data transmitted in single ReadFru command. Bigger values will
	 * improve performance. If server is returning "Invalid data field in
	 * Request." error during ReadFru command, FRU_READ_PACKET_SIZE should be
	 * decreased.
	 */
	 ...
	 * will improve performance. If server is returning "Cannot return number of
	 * requested data bytes." error during GetSdr command, CHUNK_SIZE should be
	 * decreased.
	 */
	...
	private static final String hostname = "10.0.1.245";
	...
	public static void main(String[] args) throws Exception {
		...
		while (nextRecId < MAX_REPO_RECORD_ID) {
			...
			// Search SDR repository for FruDeviceLocatorRecords, containing IDs
			// of additional FRUs (if such exist)
		...
		try {
			...
			for (FruRecord record : records) {
				...
				if (record instanceof BoardInfo) {
					System.out.println("---");
					System.out.println("board format: " + record.getClass().getSimpleName());
					...
					System.out.println("> " + bi.getBoardSerialNumber() + "; " + bi.getBoardProductName() + "; "
							+ bi.getBoardPartNumber() + "; " + bi.getBoardManufacturer() + "; " + localDateTime + " .");
					System.out.println("---");
				} else if (record instanceof ChassisInfo) {
					System.out.println("---");
					System.out.println("chassis format: " + record.getClass().getSimpleName());
					ChassisInfo ci = (ChassisInfo) record;
					System.out.println("> " + ci.getChassisPartNumber() + "; " + ci.getChassisSerialNumber() + "; "
							+ ci.getChassisType() + " !");
					System.out.println("---");
				} else if (record instanceof ProductInfo) {
					System.out.println("---");
					System.out.println("product format: " + record.getClass().getSimpleName());
					ProductInfo pi = (ProductInfo) record;
					System.out.println("> " + pi.getAssetTag() + "; " + pi.getManufacturerName() + "; "
							+ pi.getProductModelNumber() + "; " + pi.getProductName() + "; "
							+ pi.getProductSerialNumber() + "; " + pi.getProductVersion() + " .");
					System.out.println("---");
				} else {
					System.out.println("###");
					if (record instanceof PowerSupplyInfo) {
						System.out.println("multi format: " + record.getClass().getSimpleName());
						PowerSupplyInfo powerSupplyInfo = (PowerSupplyInfo) record;
						System.out.println(
								"> " + powerSupplyInfo.getCapacity() + "; " + powerSupplyInfo.getPeakVa() + " .");
					}
